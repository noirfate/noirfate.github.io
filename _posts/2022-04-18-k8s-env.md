...
## 调试k8s漏洞
1. 根据前面的步骤，制作基于metarget的编译调试镜像，需要本地准备好`[sources.list, daemon.json, metarget.tar.gz, go1.9.1.linux-amd64.tar.gz, cgdb-master.zip]`
```
FROM ubuntu:bionic
WORKDIR /root
COPY sources.list /etc/apt/
...
COPY go1.9.1.linux-amd64.tar.gz /root
RUN tar -C /usr/local -xzf go1.9.1.linux-amd64.tar.gz && ln -s /usr/local/go/bin/go /usr/bin/go && ln -s /usr/local/go/bin/gofmt /usr/bin/gofmt && go env -w GOPROXY="https://mirrors.aliyun.com/goproxy/,direct" && rm -f go1.9.1.linux-amd64.tar.gz && echo "add-auto-load-safe-path /usr/local/go/src/runtime/runtime-gdb.py" >> ~/.gdbinit
...
8. 下载[kubernets 1.9.3](https://github.com/kubernetes/kubernetes/archive/refs/tags/v1.9.3.tar.gz)
