...
```
- [x]name: `./metarget appv install [漏洞名]`中`[漏洞名]`对应的名称
- [ ]class: 应用或包名，仅显示用
- [ ]type: 漏洞类型，仅显示用
- [x]dependencies: `app`安装所需的配置文件
#### vulns_cn
云原生漏洞环境（docker、k8s、kata、kernel）配置，例如:
```yaml
...
dependencies:
  ...
  - name: kubeadm
    ...
    versions: ~
```
- [x]name: `./metarget cnv install [漏洞名]`中`[漏洞名]`对应的名称
- [ ]class: 应用名，仅显示用
- [ ]type: 漏洞类型，仅显示用
- [x]dependencies: 依赖软件的版本
